 ...
 *
 * Taken from
 * https://linuxtv.org/downloads/v4l-dvb-apis/capture-example.html by
 * Giovanni Mascellani on 2015-12-19.
 */
...
static void process_image(const void *p, int size, const struct v4l2_buffer *buf)
{
  /*if (out_buf)
  ...
  // The driver provides us with a frame timestamp, which is measured
  // on the same clock as CLOCK_MONOTONIC; so we recover the frame
  // wall clock time by adding the offset between CLOCK_MONOTONIC and
  // CLOCK_REALTIME
  assert(buf->flags & V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC);
  struct timeval buf_tv = buf->timestamp;
  struct timespec monotonic_ts, wall_ts;
  int res;

  res = clock_gettime(CLOCK_REALTIME, &wall_ts);
  if (res < 0) {
    errno_exit("clock_gettime(CLOCK_REALTIME)");
  }
  res = clock_gettime(CLOCK_MONOTONIC, &monotonic_ts);
  if (res < 0) {
    errno_exit("clock_gettime(CLOCK_MONOTONIC)");
  }

  double buf_time, monotonic_time, wall_time, corrected_time;
  buf_time = buf_tv.tv_sec + 1e-6 * buf_tv.tv_usec;
  monotonic_time = monotonic_ts.tv_sec + 1e-9 * monotonic_ts.tv_nsec;
  wall_time = wall_ts.tv_sec + 1e-9 * wall_ts.tv_nsec;
  corrected_time = wall_time - monotonic_time + buf_time;
  //printf("%f %f %f %f\n", buf_time, monotonic_time, wall_time, corrected_time);
  ...
  if (!res) {
    err("Could not write JPEG frame");
  }
}

static int read_frame_v4l2(void)
{
        ...
        case IO_METHOD_READ:
                ...
                process_image(buffers[0].start, buffers[0].length, NULL);
                break;
